...
% Plot V/W as a line in the y-direction (collapse to y-axis)
plot3(zeros(size(t)), t, zeros(size(t)), 'g-', 'LineWidth', 3, 'DisplayName', 'V/W (y-axis)');

% Plot V/U/W as a single line (again in the y-direction)
plot3(zeros(size(t)), t, zeros(size(t)) + 1, 'c-', 'LineWidth', 2, 'DisplayName', '(V/U)/(W/U) as Line');

% Add labels and grid
xlabel('x-axis');
ylabel('y-axis');
zlabel('z-axis');
title('Subspace U = span(e1), V/U, V/W, and (V/U)/(W/U)');
legend;
